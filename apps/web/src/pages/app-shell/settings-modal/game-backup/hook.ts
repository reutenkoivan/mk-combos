...
export function useSettingsGameBackup({ copy, section }: UseSettingsGameBackupInput) {
  ...
  const initialBackupFocusRef = useRef(section === settingsTabs.backup);
  const [expandedGameId, setExpandedGameId] = useState<GameId | undefined>(() =>
    section === settingsTabs.backup ? localState.resolvedActiveGameId : undefined,
  );
  ...
  useEffect(() => {
    ...
    const enteredBackup =
      section === settingsTabs.backup && previousSection !== settingsTabs.backup;
    const leftBackup = section !== settingsTabs.backup && previousSection === settingsTabs.backup;
    const shouldFocus = initialBackupFocusRef.current || enteredBackup;

    previousSectionRef.current = section;
    initialBackupFocusRef.current = false;

    if (section === settingsTabs.backup && shouldFocus) {
    ...
    () =>
      installedGames.map((business) => {
        ...
        const props: BackupCollapsibleBlockProps = {
          ...
          initialAutoExpand:
            section === settingsTabs.backup && localState.resolvedActiveGameId === business.id,
