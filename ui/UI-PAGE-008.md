...
- зміну active `game`: `MKXL` або `MK1`;
...
Settings не є first-launch gate. Початкові settings можуть бути встановлені через `UI-PAGE-002 First-Launch Setup` або через valid deep link auto-config: URL-derived `game`, URL-derived `language` і default `notation display mode = FGC`.
...
`UI-CMP-002 Game Switcher` дає змінити active game для catalog і builder entry.
...
3. Settings формує full backup JSON для local settings, custom combos і named lists.
...
6. Local settings, custom combos і named lists замінюються backup-даними.
7. Seeded combo data не змінюється.

### Deep link auto-config context

Settings може редагувати values, які з'явилися через valid deep link auto-config:
...
- Користувач може змінити `game`, `language` і `notation display mode`.
...
- Import replace змінює local settings, custom combos і named lists тільки після explicit confirmation.
...
- Зміна game на `MK1` застосовує active game до app-level state.
...
- Settings після deep link auto-config показує URL-derived game, URL-derived language і `FGC`.
...
- Confirm replace замінює local settings, custom combos і named lists backup-даними.
