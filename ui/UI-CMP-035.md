...
- `UI-CMP-035` володіє тільки presentation focus, focused move candidate, selected move group, local menu open state і temporary pick up/drop gesture state.
...
- focus step;
- focus gap;
- focus move candidate у внутрішньому `movePicker`, якщо mode підтримує builder interaction;
- request sync focused step to frame meter;
- request preview focused move candidate in frame meter;
- request open step details;
- request close local menu.

Тільки у `builderEditable`:

- request select move candidate;
- request next або previous move group;
- request candidate details, якщо page-level details підтримані;
- request append selected candidate;
- request insert selected candidate at focused gap;
- request replace focused step with selected candidate;
...
- `ComboWhiteboard` володіє presentation focus: candidate focus, step focus, gap focus, local menu, pick up/drop.
