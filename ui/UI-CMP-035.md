...
Розміщення whiteboard є робочою ділянкою builder workspace: path board є основною ділянкою, internal `movePicker` стоїть праворуч на широкому екрані або нижче path board на mobile і tablet, а markers/confirmations прив'язані до відповідної ділянки path або candidate.
...
- На `desktop` `movePicker` може бути правою sibling region; на `mobile` і `tablet` він іде нижче path board.
...
Показує candidates як grid або mobile і tablet list.
