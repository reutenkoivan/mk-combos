...
MKXL combo data contract належить `mkxl/data`. `mkxl/catalog` готує summary/context для UI variant із такими полями:
...
- На `desktop` `UI-CMP-007` і `UI-CMP-008` можуть бути сусідніми у `contextRow`; на `mobile` і `tablet` вони stack-яться згори вниз.
