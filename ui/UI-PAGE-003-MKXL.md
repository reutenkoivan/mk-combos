...
- побудову combo list для selected `character + variation`;
...
MKXL combo data contract використовує:
...
`UI-CMP-011 Combo Card` у MKXL variant має показувати:
...
- `addToList` просить focused `UI-CMP-011 Combo Card` відкрити page-level singleton `UI-CMP-021` з MKXL `character + variation` context;
...
- Add-to-list action із MKXL `UI-CMP-011 Combo Card` відкриває page-level singleton `UI-CMP-021 Add-To-List Dialog`.
