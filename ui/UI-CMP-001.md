...
Game switcher crumb емітить окремий `requestSwitchGameFromBreadcrumb` intent. App Shell застосовує analogous navigation strategy: Catalog -> target Catalog, Lists -> target Lists, Builder -> target Builder, Combo Detail -> target Catalog fallback, Settings -> лишається `/settings` із оновленим `lastActiveGameId`; post-launch switch не змінює `defaultGameId`.
