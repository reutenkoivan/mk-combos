...
`UI-CMP-006` є semantic setup form для initial game, language і notation display mode.
Він показує read-only preview лише для обраного notation mode і одну поточну submit action.
...
Розміщення є вертикальною `<form>`: game selector стоїть першим, потім language,
display mode, selected-mode preview, status і submit action.
  ...
  <SetupFormRegion slot="UI-PAGE-002 SetupSurface">
    <Stack name="SetupFormLayout">
      ...
      <NotationReferenceRegion>
        <NotationLegendTable ui="UI-CMP-037" rows="selectedModeOnly" />
      </NotationReferenceRegion>
      ...
      <SubmitActionRegion action="confirm | acknowledgeSessionOnly" />
    </Stack>
  </SetupFormRegion>
...
- Control order matches setup dependency: game, language, display mode, preview, then submit.
...
- Title всередині form має рівень `h2`; page залишається власником єдиного `h1`.
- Submit через Enter емітить ту саму semantic action, що й visible CTA.
- Якщо передано `sessionOnlyAcknowledgeLabel`, acknowledgement CTA замінює confirm CTA;
  дві primary actions одночасно не рендеряться.
...
- Game selector має visible localized label.
- Notation preview містить лише selected display mode.
- Form має рівно одну submit CTA; Enter не передає browser event назовні.
