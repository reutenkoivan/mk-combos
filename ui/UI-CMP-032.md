...
`UI-CMP-032 Breadcrumbs` показує contextual navigation trail для active working route
у верхній панелі застосунку. Query-controlled Settings modal не замінює цей trail.
...
- tablet/desktop access до `UI-PAGE-003 Catalog` через navigable `Catalog` crumb;
...
`UI-CMP-032` є inline component для `tablet` і `desktop` layout. У `mobile` layout inline breadcrumbs не монтуються; їхні navigable equivalents рендерить [`UI-CMP-033 Top Bar Dropdown Menu`](./UI-CMP-033.md).
...
- `UI-CMP-001 Global Top Bar` вирішує, чи монтувати breadcrumbs, через `responsiveMode`;
- `UI-CMP-032` рендериться в mounted tablet/desktop branch і не приховується CSS-only у mobile;
...
- Settings open state не створює breadcrumb item; underlying trail inert, доки modal open;
- mobile equivalents для breadcrumb actions належать `UI-CMP-033`.
...
- у `tablet` і `desktop` монтує `UI-CMP-032`;
- у `mobile` не монтує `UI-CMP-032`, а передає equivalent navigation model у `UI-CMP-033`;
...
- `icon` додає compact visual identity до Character і Variation/Kameo crumbs; `fallbackLabel` лишає icon slot заповненим, якщо authored asset відсутній.
...
Розміщення breadcrumbs є horizontal trail усередині tablet/desktop Top Bar: game switcher завжди перший, решта crumbs ідуть праворуч у hierarchy order, overflow affordance стоїть наприкінці.
...
- Component монтується inline на `tablet` і `desktop`; на `mobile` equivalents передаються в `UI-CMP-033`.
- Game crumb завжди лівіший за route crumbs і не змішується з `Catalog` crumb.
- Game crumb зберігає повну compact control geometry, вертикально центрується та не стискається; route crumbs truncate-яться першими.
...
- займати доступний простір у tablet/desktop Top Bar між left navigation start і controller/menu areas;
...
- не створювати hidden focus targets у mobile layout, бо компонент там не монтується.
...
- exact visual separator — `/` між game switcher і кожним route crumb.
...
Prepared trail містить рівно один current crumb, і він завжди останній. Після navigation до parent App Shell прибирає descendant crumbs замість перетворення їх на navigable items після current location.
...
`Catalog` crumb є primary tablet/desktop Top Bar access до [`UI-PAGE-003 Catalog`](./UI-PAGE-003.md). У tablet/desktop navigation layout Catalog не дублюється як global item у `UI-CMP-033`.
...
Якщо trail не вміщується у tablet/desktop Top Bar, implementation може:
...
- обрізати по вертикалі rounded corners або focus ring game switcher і route controls.

## Вхідні дані

- `responsiveMode`: `mobile`, `tablet` або `desktop`; `UI-CMP-032` монтується в `tablet` і `desktop`.
- `activeSurfaceCode`: code active working-route surface; Settings overlay не підміняє його.
...
- inline tablet/desktop breadcrumbs rendering;
...
- `UI-CMP-033` отримує mobile equivalents для game switcher і breadcrumb navigation;
- visible controller indicator, якщо є, лишається outside drawer.
...
### Inline tablet/desktop render

1. App Shell resolves active route, active game і breadcrumb model.
2. App Shell передає `responsiveMode = tablet` або `responsiveMode = desktop` у Top Bar.
...
1. App Shell або Top Bar визначає `responsiveMode = mobile`.
2. Top Bar не монтує `UI-CMP-032`.
3. Top Bar передає game switcher і breadcrumb-equivalent navigation model у `UI-CMP-033`.
4. Mobile menu забезпечує reachable equivalents для hidden inline navigation.
...
Коли Settings modal open, breadcrumb graph inert і цей flow не запускається.
...
- Character та Variation/Kameo icons є decorative поруч із readable label і не дублюють його в accessible name.
- Усі інтерактивні crumb controls, включно з Game Switcher trigger, використовують ghost presentation через semantic `subtle` emphasis; filled або outlined chrome у trail не допускається.
- Усі tablet/desktop crumb controls, current crumb, separators та icon slots зберігають одну compact vertical geometry на кожній глибині route; Character і Variation/Kameo icons використовують однаковий slot, а focus ring не обрізається navbar overflow.
- У `mobile` layout inline breadcrumbs відсутні з DOM, tab order і accessibility tree.
...
- `UI-CMP-032` монтується inline в `tablet` і `desktop`.
- У `mobile` inline breadcrumbs і inline `UI-CMP-002` не монтуються.
- First breadcrumb item у tablet/desktop layout є [`UI-CMP-002 Game Switcher`](./UI-CMP-002.md).
- Кожен visual separator у tablet/desktop trail рендериться як accessibility-hidden `/`.
- Catalog context trail показує іконку персонажа та іконку Variation/Kameo; відсутній authored asset має compact fallback glyph.
- Усі інтерактивні елементи wide і compact breadcrumb trail мають ghost presentation (`subtle` emphasis у control recipe).
- Висота tablet/desktop Top Bar не змінюється між Catalog root, Character, Variation/Kameo і Combo Detail routes.
- Game Switcher у tablet/desktop trail повністю видимий і вертикально центрований; route labels стискаються раніше за game trigger.
...
- Current crumb є останнім у trail; navigation до parent обрізає descendant crumbs.
...
- Mobile equivalents для hidden breadcrumbs лишаються у responsive drawer [`UI-CMP-033`](./UI-CMP-033.md).
- `UI-CMP-033` у tablet/desktop navigation layout не дублює breadcrumbs або game switcher.
- Breadcrumbs не містять `Named Lists`, `Builder` або `Settings` як global menu
  actions. Settings modal ніколи не стає current crumb.
...
- `WideNamedLists`;
- `WideBuilder`;
- `WideWorkingTrailUnderSettingsModal`;
- `TabletShell` із тим самим inline trail і tablet drawer без breadcrumb duplicates;
- `CurrentOnly`;
- `DisabledCrumb`;
- `LongLabels`;
- `TruncatedTrail`;
...
- focus order;
- keyboard activation;
- відсутність hidden inline breadcrumbs у mobile layout.

## Тестові сценарії

- Tablet/desktop route `/mkxl/catalog` показує `MKXL` як selected game у першому breadcrumb item.
- Tablet/desktop route `/mk1/catalog` показує `MK1` як selected game у першому breadcrumb item.
- Tablet/desktop `/mkxl/catalog` показує `Catalog` як current або safe Catalog crumb згідно з active route model.
...
- `/mkxl/catalog?settings=interface` зберігає Catalog trail, але весь breadcrumb
  region inert до Settings dismissal.
...
- Mobile layout не монтує inline `UI-CMP-032`.
- Mobile `UI-CMP-033` містить game switcher і breadcrumb-equivalent navigation.
- Tablet layout монтує inline `UI-CMP-032`, а drawer не дублює game switcher або breadcrumb navigation.
