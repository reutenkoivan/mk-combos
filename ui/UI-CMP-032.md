...
`UI-CMP-032` є inline component тільки для `desktop` layout. У `mobile` і `tablet` layout inline breadcrumbs не монтуються; їхні navigable equivalents рендерить [`UI-CMP-033 Top Bar Dropdown Menu`](./UI-CMP-033.md).
...
- `UI-CMP-032` рендериться тільки в mounted wide branch і не приховується CSS-only у mobile і tablet;
...
- mobile і tablet equivalents для breadcrumb actions належать `UI-CMP-033`.
...
- у `desktop` монтує `UI-CMP-032`;
- у `mobile` і `tablet` не монтує `UI-CMP-032`, а передає equivalent navigation model у `UI-CMP-033`;
...
- Component монтується inline тільки на `desktop`; на `mobile` і `tablet` equivalents передаються в `UI-CMP-033`.
...
- не створювати hidden focus targets у mobile і tablet layout, бо компонент там не монтується.
...
- `responsiveMode`: `mobile`, `tablet` або `desktop`; `UI-CMP-032` монтується тільки в `desktop`.
...
- mobile і tablet responsive equivalents;
...
- `UI-CMP-033` отримує mobile і tablet equivalents для game switcher і breadcrumb navigation;
...
2. App Shell передає `responsiveMode = desktop` у Top Bar.
...
1. App Shell або Top Bar визначає `responsiveMode = mobile` або `responsiveMode = tablet`.
...
- У `mobile` і `tablet` layout inline breadcrumbs відсутні з DOM, tab order і accessibility tree.
...
- `UI-CMP-032` монтується inline тільки в `desktop`.
- У `mobile` і `tablet` inline breadcrumbs і inline `UI-CMP-002` не монтуються.
...
- Compact equivalents для hidden breadcrumbs лишаються у responsive drawer [`UI-CMP-033`](./UI-CMP-033.md).
...
- focus order;
- keyboard activation;
- відсутність hidden inline breadcrumbs у mobile і tablet layout.
