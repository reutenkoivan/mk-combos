...
- selected і focused state зберігаються під час переходу між wide і mobile і tablet layout;
- усі non-placeholder slots лишаються reachable;
- mobile і tablet layout не має створювати overlap або недоступні controls.

## Анатомія

Розміщення є picker surface: label стоїть над roster grid, grid містить slots у prepared `row`/`column` або `responsiveOrder`, status region стоїть нижче grid.
...
- На `desktop` grid повторює prepared in-game roster positions; на `mobile` і `tablet` slots читаються за `responsiveOrder`.
...
- exact in-game slot placement на `desktop`;
- adaptive mobile і tablet layout через `responsiveOrder`;
...
- на mobile і tablet layout options ідуть за `responsiveOrder`.
...
- mobile і tablet layout може пропустити placeholder, якщо це не ламає logical order.
...
- navigation іде за `responsiveOrder`;
...
- Passive layout reflow на mobile і tablet не має перехоплювати focus.
