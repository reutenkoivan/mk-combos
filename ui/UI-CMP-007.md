...
- selected, focused, disabled і placeholder state roster slots;
- keyboard/controller navigation усередині roster grid;
...
Layout registry і character metadata готові.
...
- Layout не покладається тільки на колір для selected, focused або disabled state.
