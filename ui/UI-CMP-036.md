...
5. desktop roving segment triggers безпосередньо у timeline або compact roving segment navigation toolbar;
6. prepared grouped legend у нижній частині available state;
7. invalid/unavailable annotations;
8. portal-rendered anchored details popover для active segment.

Grid має фіксовану читабельну ширину клітинки. Довга timeline прокручується тільки горизонтально; legend, summary та annotations стоять поза scroller, а details popover рендериться через portal і позиціонується від active timeline span. На desktop легенда йде одразу після timeline; на mobile/tablet — після compact segment navigation. Компонент не використовує fixed height або `overflow-y-hidden`, тому контент не обрізається у вузькому чи низькому контейнері.
...
- відкритий segment має зовнішню selection рамку `2px`, `scale(1.06)` без reflow і піднятий stacking order;
...
Один roving focus model використовує різні presentation surfaces без дубльованих focus targets:

- на `desktop` visual frame segments є click/keyboard triggers у самій timeline;
- на `mobile`/`tablet` visual spans лишаються точними anchors, а focus належить compact `44×44px` segment controls;
- `ArrowLeft`/`ArrowRight` переходять між segments;
- `Enter`/`confirm` або click відкриває click-persistent anchored details popover;
- повторний click active segment не закриває popover; вибір іншого segment переносить selection, content і arrow anchor;
- перший `Escape`, поки details відкриті, закриває popover і лишає або повертає focus на source trigger;
- наступний keyboard `Escape` на source trigger із закритими details очищає `focusedSegmentId`, прибирає DOM focus/focus ring і лишає перший segment єдиним `tabIndex=0` для наступного входу;
- controller `back`, outside press і Close зберігають власні page/popover focus-recovery semantics;
...
- Details мають non-modal popover semantics, accessible title/description, arrow до active span і live announcement; це не hover-only tooltip.
- Horizontal timeline scroll не приховує vertical content.
- Focus recovery після close детерміновано повертається на source trigger; окремий наступний `Escape` виходить із timeline focus без приховування ring на DOM-focused control.
...
- Категоризована легенда показує `Phases` і `Meta` після active segment-control surface; uncategorized legacy items лишаються підтриманими.
- Довгі grids мають тільки horizontal overflow; summary/annotations не обрізаються, а portal details не кліпаються scroller-ом.
...
- Легенда йде після desktop timeline або compact segment navigation і містить доступно названі `Phases` та `Meta` groups.
...
- Desktop timeline triggers, compact roving controls, anchored popover open/retarget/close і focus recovery працюють з keyboard/controller.
- Selection рамка, `scale(1.06)` та arrow anchor лишаються видимими без зміни grid geometry або clipping.
...
- `mobile` використовує vertical-first composition, compact segment controls не менші за `44×44px` і popover, anchored до відповідного visual span;
- `tablet` використовує hybrid composition з тими самими compact controls;
- `desktop` використовує повноширинний meter із compact aligned rows та roving triggers безпосередньо у timeline;
