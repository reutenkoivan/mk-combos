...
- Пов'язані компоненти: [`UI-CMP-001`](./UI-CMP-001.md)
- Вкладені компоненти: [`UI-CMP-005`](./UI-CMP-005.md), `UI-CMP-032`, `UI-CMP-033` всередині `UI-CMP-001`
...
`UI-CMP-001 Global Top Bar` є постійною верхньою панеллю. Вона показує поточну версію гри, controller indicator area, breadcrumbs і right-pinned dropdown menu.
...
- давати доступ до lists, builder, backup actions і settings через dropdown menu;
...
- dropdown action для lists, builder, backup management або settings;
...
- не змінює `game`, `language` або `notation display mode`;
