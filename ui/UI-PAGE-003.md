...
```text
UI-PAGE-003 Catalog
  └─ (inside UI-PAGE-001 active route slot) Catalog root
     ...
     │  └─ (below) UI-CMP-013 Filter Control Group
     │     ├─ (top) filterHeader
     │     └─ (below) filterBody
     ├─ (below) UI-CMP-010 Combo List
     │  └─ (inside list) UI-CMP-011 Combo Card
     │     └─ (inside card) UI-CMP-015 Notation Renderer
     ├─ (below) System state area
...
- На `wide13_6Plus` context pickers можуть стояти поруч; на `compact` вони stack-яться згори вниз.
