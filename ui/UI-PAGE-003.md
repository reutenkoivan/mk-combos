...
- Пов'язані компоненти: `UI-CMP-007`, [`UI-CMP-010`](./UI-CMP-010.md), `UI-CMP-011`, `UI-CMP-012`, `UI-CMP-013`, `UI-CMP-015`, `UI-CMP-021`, `UI-CMP-029`, `UI-CMP-030`
...
`UI-PAGE-003 Catalog` є route-level сторінкою для перегляду seeded combo data в межах active game.

Цей документ описує shared contract Catalog surface:

- прийняти already applied app-level settings із `UI-PAGE-001 App Shell`;
...
Catalog працює тільки з already applied app-level settings:
...
- читання active app settings, переданих через `UI-PAGE-001 App Shell`;
- вибір або відновлення game-specific variant за active `game`;
...
- побудову visible combo list із seeded combo data для поточного context;
- застосування live optional filter state до visible combo list;
- відкриття detail route для selected combo;
- прийом add-to-list request від `UI-CMP-011 Combo Card`;
...
Catalog root є page-level container, який рендериться в slot активної сторінки `UI-PAGE-001 App Shell`.
...
System state не має приховувати валідні recovery actions: clear filters, choose character, choose game-specific context або return to Catalog root.
...
- focus character slot за `row`/`column` або `compactOrder`;
...
- не існує як repeated child instance усередині кожної `UI-CMP-011 Combo Card`;
...
- receive add-to-list request from `UI-CMP-011 Combo Card`;
...
- доступний fallback до Catalog root або valid nearest context;
...
- retry, clear filters або return to Catalog root доступні відповідно до причини.
...
2. App-level state застосовує game, language і notation display mode.
3. `UI-PAGE-001 App Shell` відкриває `UI-PAGE-003 Catalog`.
4. Catalog отримує active settings і seeded combo data.
5. Catalog вибирає active variant за game і показує character selection.
...
2. Якщо first-launch marker відсутній, але URL дозволяє valid auto-config, App Shell застосовує URL-derived settings і не показує `UI-PAGE-002`.
3. Catalog визначає active variant за route/app-level game.
4. Catalog перевіряє character і game-specific context.
...
4. Catalog live-застосовує configuration до visible combo list.
...
3. Catalog передає combo id і list context у app-level routing.
...
2. `UI-CMP-011 Combo Card` емітить page-level add-to-list request із combo context.
...
4. Builder отримує initial `movePath` і variant context.
...
- `addToList`: попросити focused `UI-CMP-011 Combo Card` емітити request для відкриття page-level singleton `UI-CMP-021`;
...
- виконувати add-to-list без focused `UI-CMP-011 Combo Card`;
...
- Combo card відкриває `UI-PAGE-004 Combo Detail`.
- Add-to-list action з focused `UI-CMP-011 Combo Card` відкриває page-level singleton `UI-CMP-021 Add-To-List Dialog`.
...
- `addToList` через controller просить focused `UI-CMP-011 Combo Card` відкрити page-level singleton add-to-list dialog.
...
- Точний route format для catalog deep links буде визначено під час реалізації `apps/web`.
