...
Default language визначається на app boundary: browser locale `uk` або `uk-*` дає
`UA`, усі інші locale дають `EN`. Default notation display mode — `FGC`.

Valid route-prefixed deep link не рендерить `UI-PAGE-002`. Якщо URL містить installed `gameId`, App Shell застосовує URL-derived active game, browser-derived default language і `notation display mode = FGC`, створює first-launch completion marker і одразу відкриває target surface без confirmation.
...
На desktop сторінка має двоколонкову композицію: короткий summary і data notice
стоять зліва, form — справа. На mobile ті самі частини утворюють один послідовний
reading flow.

```jsx
<FirstLaunchSetupPage ui="UI-PAGE-002">
  <SetupSurface slot="UI-PAGE-001 active route">
    <ResponsiveLayout desktop="two-column" mobile="single-column">
      <Stack name="SetupSummary">
        <CapabilitiesSummary items={3} />
        <LocalDataNotice />
      </Stack>
      ...
      <FirstLaunchSetupForm ui="UI-CMP-006">
        ...
        <NotationLegendTable ui="UI-CMP-037" rows="selected-display-mode-only" />
        <ConfirmationAction />
...
- Session-only message стоїть у form перед єдиною primary acknowledgement action.
...
Summary складається рівно з трьох коротких переваг: пошук комбо в каталогах,
збереження/створення власних комбо та робота з геймпадом і per-game backup.
Окремий data notice пояснює, що списки й custom combos лишаються в поточному
браузері, доки користувач не створить backup.
...
[`UI-CMP-037`](./UI-CMP-037.md) показує один read-only preview row тільки для
поточного `notation display mode`. Він не є required setting і не впливає на
readiness confirmation action.
...
Session-only state показує зрозуміле warning про втрату choices після reload або
закриття сторінки. Звичайна confirmation action замінюється однією primary action
`Continue without saving` / `Продовжити без збереження`; дві конкуруючі CTA не
рендеряться одночасно.
...
- Capabilities summary містить три короткі user-value пункти й окремий local-data notice.
...
- First Launch рендерить [`UI-CMP-037 Notation Legend Table`](./UI-CMP-037.md) як
  selected-mode-only preview поруч із display mode selector.
...
- Notation preview показує тільки selected FGC, PlayStation або Xbox row через UI-owned SVG icons.
...
- Browser locale fallback належить app-level settings logic: `uk`/`uk-*` -> `UA`, інші locale -> `EN`.
