...
- дати вибрати початкові settings: `game`, `language` і `notation display mode`;
...
- перевести користувача до `UI-PAGE-003 Catalog` після завершення setup.
...
Valid deep link не рендерить `UI-PAGE-002`. Якщо URL дозволяє вирахувати `game` і `language`, App Shell застосовує URL-derived `game`, URL-derived `language`, default `notation display mode = FGC`, створює first-launch completion marker і одразу відкриває target surface без confirmation.
...
- filters для пошуку потрібних маршрутів;
...
6. App відкриває `UI-PAGE-003 Catalog`.

### Deep link bypass

1. App отримує target route з URL.
2. App Shell визначає, що URL є valid deep link і дозволяє вирахувати `game` та `language`.
3. App Shell не рендерить `UI-PAGE-002`.
4. App Shell застосовує URL-derived `game`, URL-derived `language` і default `notation display mode = FGC`.
...
- Після completion root first launch переходить до `UI-PAGE-003 Catalog`.
- Deep-link first launch застосовує URL-derived `game`, URL-derived `language`, `FGC` і completion marker без confirmation.
...
- Вибір `UA`, `MK1`, `Xbox` і confirmation застосовує settings.
...
- Після confirmation root flow відкриває `UI-PAGE-003 Catalog`.
- Valid deep link у fresh browser state не показує setup і одразу відкриває target surface.
- Valid deep link застосовує URL-derived `game`, URL-derived `language` і `FGC` без confirmation.
