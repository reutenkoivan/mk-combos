...
const validateReferences = (issues: MkxlDataValidationIssue[]) => {
  ...
  const variationIds = new Set(mkxlVariations.map((variation) => variation.id));
  const moveById = new Map(mkxlMoves.map((move) => [move.id, move]));
  const stageIds = new Set(mkxlStages.map((stage) => stage.id));
  const interactableIds = new Set(mkxlInteractableIds);

  assertUnique(
    mkxlCharacters.map((character) => character.id),
    "Character",
    issues,
  );
  assertUnique(
    mkxlVariations.map((variation) => variation.id),
    "Variation",
    issues,
  );
  assertUnique(
    mkxlMoves.map((move) => move.id),
    "Move",
    issues,
  );
  assertUnique(
    mkxlSeededCombos.map((combo) => combo.id),
    "Combo",
    issues,
  );
  assertUnique(
    mkxlStages.map((stage) => stage.id),
    "Stage",
    issues,
  );
  ...
  for (const variation of mkxlVariations) {
    if (!characterIds.has(variation.characterId)) {
      issues.push(
        createIssue(
  ...
  for (const movelist of mkxlMovelists) {
    if (!characterIds.has(movelist.characterId) && movelist.characterId !== generalMoveOwnerId) {
      issues.push(
        createIssue(
  ...
  for (const move of mkxlMoves) {
    if (!characterIds.has(move.characterId) && move.characterId !== generalMoveOwnerId) {
      issues.push(
        createIssue(
  ...
  for (const combo of mkxlSeededCombos) {
    if (!characterIds.has(combo.characterId)) {
      issues.push(
        createIssue(
    ...
    if (!variationIds.has(combo.variationId)) {
      issues.push(
        createIssue(
    ...
    for (const [index, step] of combo.route.entries()) {
      const move = moveById.get(step.moveId);
    ...
    const expectedMovePath = combo.route.map((step) => step.moveId);
    ...
    const expectedNotation = combo.route.map((step) => moveById.get(step.moveId)?.notation ?? []);
    ...
    if (combo.stageContext.kind === "stageSpecific") {
      if (!stageIds.has(combo.stageContext.stageId)) {
        issues.push(
          createIssue(
      ...
      for (const interactableId of combo.stageContext.interactableIds) {
        if (!interactableIds.has(interactableId)) {
          issues.push(
            createIssue(
...
const validateMovelistFileRegistry = (issues: MkxlDataValidationIssue[]) => {
  const characterIds = new Set(mkxlCharacters.map((character) => character.id));
  const variationsById = new Map(mkxlVariations.map((variation) => [variation.id, variation]));
...
const validateMoveTreeRegistry = (issues: MkxlDataValidationIssue[]) => {
  ...
  const movelistByCharacter = new Map(
  ...
  for (const [registryKey, entry] of Object.entries(mkxlMoveTreeRegistry)) {
    ...
    const moveTreeRoot = entry.moves as Record<string, unknown>;
    ...
    const aggregateMoveIds = new Set((movelist?.movelist ?? []).map((move) => move.id));
    const treeMoveIds = new Set(collectMoveTreeMoves(moveTreeRoot).map((move) => move.id));
    ...
    for (const variation of mkxlVariationsByCharacterId[entry.characterId] ?? []) {
      ...
      const variationBranch = moveTreeRoot[variationKey];
...
const validateComboAndGraphCoverage = (issues: MkxlDataValidationIssue[]) => {
  ...
  const graphVariationIds = new Set(mkxlVariationGraphs.map((graph) => graph.variationId));
  for (const variation of mkxlVariations) {
